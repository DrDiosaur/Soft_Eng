...
namespace Musicorum.Services.Implementations
{
    public class VideoService : IVideoService
    {
        ...
        {
            IList<VideoModel> videos = this.db.Videos.Where(x => x.EventId == eventId).ProjectTo<VideoModel>().ToList();

            foreach(VideoModel video in videos)
            {
                video.VideoUrl = FileHelper.GetVideoFilePath(video.VideoAsBytes, video.Id);
            }
